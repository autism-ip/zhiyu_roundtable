...
function CalligraphyTitle({ children, className = "", size = "lg" }: { children: React.ReactNode; className?: string; size?: "sm" | "md" | "lg" | "xl" | "hero" }) {
  const sizeClasses = { sm: "text-2xl md:text-3xl", md: "text-3xl md:text-4xl", lg: "text-3xl md:text-5xl lg:text-6xl", xl: "text-4xl md:text-6xl lg:text-7xl", hero: "text-4xl md:text-6xl lg:text-7xl" };
...
function FeatureCard({ icon: Icon, title, description, features, accentColor, delay = 0, href, onClick }: { icon: React.ElementType; title: string; description: string; features: string[]; accentColor: string; delay?: number; href?: string; onClick?: () => void }) {
...
function AuxiliaryCard({ icon: Icon, title, href, accentColor }: { icon: React.ElementType; title: string; href: string; accentColor: string }) {
  const ref = useRef(null);
  ...
  return (
    <motion.div ref={ref} variants={cardVariants} initial="hidden" animate={isInView ? "visible" : "hidden"}>
      <Link
        href={href}
        className="flex items-center gap-4 p-5 group transition-all duration-300 hover:-translate-y-1"
        style={{ backgroundColor: colors.paper, borderColor: `${accentColor}20`, borderRadius: '10px', borderWidth: '1px' }}
      >
        <motion.div
          className="w-12 h-12 rounded-lg flex items-center justify-center transition-colors duration-300 group-hover:scale-110"
          style={{ backgroundColor: `${accentColor}15` }}
        >
          <Icon className="w-5 h-5" style={{ color: accentColor }} />
        </motion.div>
        <span className="text-base font-medium transition-colors duration-300" style={{ color: colors.ink }}>{title}</span>
      </Link>
    </motion.div>
  );
}
  ...
  return (
    <div className="min-h-screen relative" style={{ backgroundColor: colors.background }}>
      ...
      <div className="relative z-10">
        {/* Hero Section */}
        <section ref={heroRef} className="container mx-auto px-4 py-20 md:py-32 text-center">
          <motion.div initial="hidden" animate="visible" variants={pageVariants}>
            ...
            {/* CTA 按钮 - 直接使用 Link + motion.a */}
            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-4 justify-center">
              <motion.a
                href="/rounds"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="group relative inline-flex items-center h-14 px-10 text-sm font-medium tracking-wide overflow-hidden transition-all duration-300 no-underline"
        ...
        <section className="container mx-auto px-4 py-24 md:py-32">
          ...
          <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-20px" }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            <FeatureCard icon={Telescope} title="伯乐层" description="发现高价值连接" features={["AI Agent 参与圆桌讨论", "分析互补性与生成性", "生成知遇卡匹配"]} accentColor={colors.daiQing} delay={0} href="/rounds" />
            <FeatureCard icon={MessageSquare} title="争鸣层" description="验证合作可行性" features={["结构化对练与压力测试", "识别风险领域", "输出关系类型建议"]} accentColor={colors.zhuHong} delay={0.1} href="/matches" />
            <FeatureCard icon={Handshake} title="共试层" description="低成本关系落地" features={["最小化协作任务设计", "真实协作场景验证", "长期关系追踪"]} accentColor={colors.jiangZi} delay={0.2} href="/matches" />
          </motion.div>
        </section>

        {/* CTA */}
        <section className="container mx-auto px-4 py-16 md:py-24">
          <motion.div initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative max-w-2xl mx-auto text-center p-6 md:p-12 lg:p-16 group cursor-pointer" style={{ backgroundColor: colors.paper, border: `1px solid ${colors.gold}30` }}>
            {/* 角落 - 更醒目 */}
            ...
            <div className="relative">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
                <motion.a
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="group relative inline-flex items-center h-12 px-12 text-sm font-medium tracking-wide transition-all duration-300 no-underline"
