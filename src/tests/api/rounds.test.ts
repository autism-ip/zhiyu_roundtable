...
// ===========================================
// Mock lucide-react - must be hoisted
// ===========================================
...
describe('Rounds API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    isAuthenticated = true; // Reset to authenticated state
  });
  ...
  describe('POST /api/rounds', () => {
    ...
    it('未登录时应返回 401', async () => {
      // Arrange - modify auth state to simulate not logged in
  ...
  describe('POST /api/rounds/[id]/start', () => {
    it('应开始圆桌', async () => {
      ...
      mockRoundService.getRound.mockResolvedValue(mockRound);
      mockRoundService.startRound.mockResolvedValue({ ...mockRound, status: 'ongoing' });
      mockAuditLogger.logRoundAction.mockResolvedValue({});
  ...
  describe('POST /api/rounds/[id]/complete', () => {
    it('应结束圆桌', async () => {
      ...
      mockRoundService.getRound.mockResolvedValue(mockRound);
      mockRoundService.completeRound.mockResolvedValue({ ...mockRound, status: 'completed' });
      mockAuditLogger.logRoundAction.mockResolvedValue({});
