...
export class CardGenerator {
   ...
   * @returns 知遇卡候选列表
   */
  async generateMatches(round: DbRound): Promise<MatchCandidate[]> {
    // 检查圆桌讨论时长
   ...
   * 计算圆桌讨论时长
   */
  private calculateRoundDuration(round: DbRound): number {
   ...
   * 获取圆桌参与者
   */
  private async getRoundParticipants(roundId: string): Promise<DbUser[]> {
    const { data } = await supabaseAdmin
      .from('round_participants')
      .select('user:users(*)')
      .eq('round_id', roundId);

    return (data || []).map(p => p.user as DbUser).filter(Boolean);
  }
...
- 期望连接类型：${(userA.connection_types || []).join(', ')}

用户B：
- 名字：${userB.name || '未知'}
- 兴趣：${(userB.interests || []).join(', ')}
- 期望连接类型：${(userB.connection_types || []).join(', ')}
